...
async function startServer() {
  ...
  app.get("/api/catalog", async (req, res) => {
    ...
    try {
      ...
      // If file doesn't exist, try to read from public/catalog.json as fallback
      try {
        const fallbackPath = path.join(__dirname, "public", "catalog.json");
        const fallbackData = await fs.readFile(fallbackPath, "utf-8");
        // Also save it to data/catalog.json for future
        await fs.writeFile(catalogPath, fallbackData, "utf-8");
      ...
      for (const file of files) {
        ...
        if (['.md', '.txt'].includes(ext)) {
          ...
          if (content.toLowerCase().includes(query.toLowerCase())) {
            results.push(`/docs/${file}`);
          }
        }
        // Note: For PDF/Docx we would need more complex parsing, 
        // but for now we support text-based formats as requested.
      }
